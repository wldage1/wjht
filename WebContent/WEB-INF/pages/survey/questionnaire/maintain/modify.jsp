...
KindEditor.ready(function(K) {
        ...
        var svalue =  $('#queStartTime').val();
        if(svalue != null && svalue != ''){
            $('#queStartTime').val(svalue.substring(0,19));
        }
        var evalue =  $('#queEndTime').val();
        if(evalue != null && evalue != ''){
        	$('#queEndTime').val(evalue.substring(0,19));
        }
});
...
$(document).ready(function(){
	...
	$("#uploadify").uploadify({
      ...
      'queueID'        : 'fileQueue',
      'multi'          : false,
      'formData'       : {'queImgId':'${questionnaire.queImgIpad}','queId':'${questionnaire.id }'},//第一次上传的时候，需要删除表中信息
	...
	$("#uploadify1").uploadify({
      ...
      'queueID'        : 'fileQueue1',
      'multi'          : false,
      'formData'       : {'queImgId':'${questionnaire.queImgIphone}','queId':'${questionnaire.id }'},//第一次上传的时候，需要删除表中信息
      'sizeLimit'		 : 86400 ,     //这里是文件最大值 byte
      'fileTypeDesc'   : "jpg文件",               
		'fileTypeExts'   : "*.jpg;", //控制可上传文件的扩展名，启用本项时需同时声明fileDesc           		
      'buttonText'     : '<msg:message code='info.prompt.select'/>',   
...
function deleteImg(id,type,queId){
	var url = "deleteImgFile.json";
	var jsonDate = {queImgId:id,queImgType:type};
	if(queId != null && queId != '' && queId != 'undefined'){
		jsonDate['queId'] = queId;
	}
	$.ajax({
		url:url,
		type:'post',
...
<body class="skinMain">
<form:form method="post" action="update" commandName="questionnaire">
<form:hidden path="c"/>	
<form:hidden path="id"/>
			...
			<table width="100%" border="0" cellspacing="0" cellpadding="0"
				...
				<tr>
							...
							<td width="85%" class="skinLayout_lable"><form:hidden
								...
								<div id="imgIpadContainer">
								<c:if test="${questionnaire.queImgIpad != null  && questionnaire.queImgIpad != ''}">
								<div id="${imgIpadFileId}" class='imgInfoShowIpad'><span>
										<span style="color:blue;margin-right:10px;">『${questionnaire.queImgIpad}』</span>
										<a href="#"   onclick="deleteImg('${imgIpadFileId}','Ipad','${questionnaire.id }')" style="margin-right:10px;"><msg:message code="button.delete"/></a>
										<a href="#" style="margin-right:10px;" onclick="viewQueImg('${imgPathIpad}/','${questionnaire.queImgIpad}')"><msg:message code="product.downLoadPath"/></a>
										<c:if test="${openStatus == 'open' }">
										<a href="#"  onclick="viewSynUploadImg('${synUploadUrl }/${imgPathIpad}/${questionnaire.queImgIpad}')" ><msg:message code="product.downLoadPathSyn"/></a>
										</c:if>
										</div>
								</c:if>
								</div></td>
			     ...
			     <tr>
							...
							<td width="85%" class="skinLayout_lable"><form:hidden
								...
								<div id="imgIphoneContainer">
								<c:if test="${questionnaire.queImgIphone != null  && questionnaire.queImgIphone != ''}">
								<div id="${imgIphoneFileId}" class='imgInfoShowIphone'><span>${queImgIphone}
										<span style="color:blue;margin-right:10px;">『${questionnaire.queImgIphone}』</span>
										<a href="#"   onclick="deleteImg('${imgIphoneFileId}','Iphone','${questionnaire.id }')" style="margin-right:10px;"><msg:message code="button.delete"/></a>
										<a href="#" style="margin-right:10px;" onclick="viewQueImg('${imgPathIphone}/','${questionnaire.queImgIphone}')"><msg:message code="product.downLoadPath"/></a>
										<c:if test="${openStatus == 'open' }">
											<a href="#"  onclick="viewSynUploadImg('${synUploadUrl }/${imgPathIphone}/${questionnaire.queImgIphone}')" ><msg:message code="product.downLoadPathSyn"/></a>
										</c:if>
										</div>
								</c:if>
