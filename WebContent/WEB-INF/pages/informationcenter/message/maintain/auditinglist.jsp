...
$(document).ready(function(){
    jQuery("#auditingList").jqGrid({
        treeGridModel: 'adjacency',
        url: 'grid.json?auditState=0' ,
        ExpandColumn: 'name',
        ExpandColClick: true,
        datatype: 'json',
    	...
    	gridComplete: function(){
    		var ids = jQuery("#auditingList").jqGrid('getDataIDs');
    		for(var i=0;i < ids.length;i++){
    			var id = ids[i];
    			var content = "";
    			<c:if test="${!empty auditing_details}">
    		    <security:authorize ifAnyGranted="${auditing_details.code}">	
    			content += "<a href='#' id='" + id + "' class='shortcut_${auditing_details.index}' title='${auditing_details.name}'>";
    			content += "<img src='${base}/${common}/${style}/images/icon/detail.png' weight='18' height='18' border='0' align='absmiddle'/>${auditing_details.name}";
    			content += "</a>";
    		    </security:authorize>
    		    </c:if>
    		    <c:if test="${!empty auditing_audit}">
    		    <security:authorize ifAnyGranted="${auditing_audit.code}">
    		    content += "<a href='#' id='" + id + "' class='shortcut_${auditing_audit.index}' title='${auditing_audit.name}'>";
    		    content += "<img src='${base}/${common}/${style}/images/icon/audit.png' weight='18' height='18' border='0' align='absmiddle'/>${auditing_audit.name}";
    		    content += "</a>";	
    		    </security:authorize>
    		    </c:if> 
    		    
    		    jQuery("#auditingList").jqGrid('setRowData',ids[i],{act:"<div class='jqgridContainer'>" + content + "</div>"});
    		}	
    	},
    	loadComplete:function(){
    		$(".shortcut_${auditing_details.index}").click(function(){
    	    	var rowid = $(this).attr("id");
    			window.location.href = "${base}${auditing_details.controller}?c=${auditing_details.code}&id="+rowid;
    	    });
    		$(".shortcut_${auditing_audit.index}").click(function(){
    	    	var rowid = $(this).attr("id");
    			window.location.href = "${base}${auditing_audit.controller}?c=${auditing_audit.code}&id="+rowid;
    	    });
    	},
        caption:'<msg:message code="message.auditinglist"/>',
         toolbar: [false,"top"]
    });
});
function formSubmit(){
