...
<table width="100%" border="0" cellspacing="1" cellpadding="0" class="skinMain">
	...
	<tr>
		<td width="100%">
			...
			<input type="hidden" name="id" value="${shareplugin.id}"/>
			...
			<table width="100%" border="0" cellspacing="0" cellpadding="0" class="skinLayout">		
					...
					<tr>
						...
						<c:forEach var="item" items="${devicetypes}">
							<input type="checkbox" name="types" <c:forEach var="pt" items="${plugintypes}"><c:if test="${pt.type == item.key}">checked="checked"</c:if></c:forEach> value="${item.key}" style="width:15px;height:15px;border:0px;"/>&nbsp;&nbsp;<label>${item.value}</label>&nbsp;&nbsp;
						</c:forEach>
