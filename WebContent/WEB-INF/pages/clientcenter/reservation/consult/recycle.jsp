...
$(document).ready(function(){
    jQuery("#consultList").jqGrid({
    	...
    	gridComplete: function(){
    		...
    		for(var i=0;i < ids.length;i++){
    			var id = ids[i];
    			var content = "";
    			<c:if test="${!empty consult_restore}">
    			content += "<a href='javascript:void(0);' id='" + id + "' class='shortcut_${consult_restore.index}'";
    			content += "title='${consult_restore.name}' >";
    			content += "<img src='${base}/${common}/${style}/images/icon/restore.png' weight='18' height='18' border='0' align='absmiddle'/>${consult_restore.name}";
    			content += "</a>";
    			content += "&nbsp;|&nbsp;";
    			</c:if> 
    			<c:if test="${!empty consult_restore_delete}">
    		    <security:authorize ifAnyGranted="${consult_restore_delete.code}">	
    			content += "<a href='javascript:void(0);' id='" + id + "' class='shortcut_${consult_restore_delete.index}'";
    			content += "title='${consult_restore_delete.name}' >";
    			content += "<img src='${base}/${common}/${style}/images/icon/delete.png' weight='18' height='18' border='0' align='absmiddle'/>${consult_restore_delete.name}";
    			content += "</a>";
    			 </security:authorize>
    			</c:if> 
    	...
    	loadComplete:function(){
    		$(".shortcut_${consult_restore.index}").click(function(){
    			...
    			window.parent.Boxy.confirm("<msg:message code='info.prompt.restore.confirm'/>",  function(){
    				var url = "${base}${consult_restore.controller}.json?c=${consult_restore.code}&state=0&ids="+rowid+"&prompt=name&name="+encodeURIComponent(encodeURIComponent(data.name));
    				$.ajax({
    					url:url,
    					type:'post',
    					timeout:'60000',
    	    ...
    	    $(".shortcut_${consult_restore_delete.index}").click(function(){
    	    	var rowid = $(this).attr("id");
    	    	var data = jQuery("#consultList").jqGrid("getRowData",rowid);
    			window.parent.Boxy.confirm("<msg:message code='info.prompt.delete.confirm'/>",  function(){
    				var url = "${base}${consult_restore_delete.controller}.json?c=${consult_restore_delete.code}&ids="+rowid+"&prompt=name&name="+encodeURIComponent(encodeURIComponent(data.name));
    				$.ajax({
    					url:url,
    					type:'post',
    					timeout:'60000',
    ...
    <c:if test="${!empty consult_restore}">
    <security:authorize ifAnyGranted="${consult_restore.code}">	 
    var $ea = $("<a></a>").attr("href","javascript:void(0)")
			  .attr("id","${consult_restore.index}")
			  .append($("<img/>").attr("src","${base}/${common}/${style}/images/icon/restore.png")
			  .attr("width","18").attr("height","18").attr("border","0")
			  .attr("border","0").attr("align","absmiddle"))
			  .append("${consult_restore.name}");
	$("#t_consultList").append("&nbsp;&nbsp;").append($("<span></span>").attr("class","jqgridContainer").append($ea));    
    $("#${consult_restore.index}","#t_consultList").click(function(){
		...
		if (selrow != null && selrow != ""){
			window.parent.Boxy.confirm("<msg:message code='info.prompt.restore.confirm'/>",  function(){
				var url = "${base}${consult_restore.controller}.json?c=${consult_restore.code}&state=0&ids="+selrow+"&prompt=name&name="+encodeURIComponent(encodeURIComponent(names));
				$.ajax({
					url:url,
					type:'post',
					timeout:'60000',
    ...
    <security:authorize ifAnyGranted="${consult_restore_delete.code}">	 
    var $ea = $("<a></a>").attr("href","javascript:void(0)")
			  .attr("id","${consult_restore_delete.index}")
			  .append($("<img/>").attr("src","${base}/${common}/${style}/images/icon/delete.png")
			  .attr("width","18").attr("height","18").attr("border","0")
			  .attr("border","0").attr("align","absmiddle"))
			  .append("${consult_restore_delete.name}");
	$("#t_consultList").append("&nbsp;&nbsp;").append($("<span></span>").attr("class","jqgridContainer").append($ea));    
    $("#${consult_restore_delete.index}","#t_consultList").click(function(){
    	var selrow = jQuery("#consultList").jqGrid("getGridParam","selarrrow");
    	var names = "";
    	for(var i=0;i < selrow.length;i++){
    		var id = selrow[i];
		...
		if (selrow != null && selrow != ""){
			window.parent.Boxy.confirm("<msg:message code='info.prompt.restore.confirm'/>",  function(){
				var url = "${base}${consult_restore_delete.controller}.json?c=${consult_restore_delete.code}&ids="+selrow+"&prompt=name&name="+encodeURIComponent(encodeURIComponent(names));
				$.ajax({
					url:url,
					type:'post',
					timeout:'60000',
