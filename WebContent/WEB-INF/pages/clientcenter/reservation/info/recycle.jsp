...
$(document).ready(function(){
    jQuery("#reservationList").jqGrid({
    	...
    	gridComplete: function(){
    		...
    		for(var i=0;i < ids.length;i++){
    			...
    			var content = "";
    			<c:if test="${!empty res_restore}">
    		    <security:authorize ifAnyGranted="${res_restore.code}">
    			content += "<a href='javascript:void(0);' id='" + id + "' class='shortcut_${res_restore.index}'";
    			content += "title='${res_restore.name}' >";
    			content += "<img src='${base}/${common}/${style}/images/icon/restore.png' weight='18' height='18' border='0' align='absmiddle'/>${res_restore.name}";
    			content += "</a>";
    			</security:authorize>
    			</c:if>
    			<c:if test="${!empty res_restore_delete}">
    		    <security:authorize ifAnyGranted="${res_restore_delete.code}">
    			content += "<a href='javascript:void(0);' id='" + id + "' class='shortcut_${res_restore_delete.index}'";
    			content += "title='${res_restore_delete.name}' >";
    			content += "<img src='${base}/${common}/${style}/images/icon/delete.png' weight='18' height='18' border='0' align='absmiddle'/>${res_restore_delete.name}";
    			content += "</a>";
    			</security:authorize>
    			</c:if>
    	...
    	loadComplete:function(){
    	    $(".shortcut_${res_restore.index}").click(function(){
    			...
    			window.parent.Boxy.confirm("<msg:message code='info.prompt.restore.confirm'/>",  function(){
    				var url = "${base}${res_restore.controller}.json?c=${res_restore.code}&delState=0&ids="+rowid+"&prompt=name&name="+encodeURIComponent(encodeURIComponent(data.name));
    				$.ajax({
    					url:url,
    					type:'post',
    					timeout:'60000',
    	    ...
    	    $(".shortcut_${res_restore_delete.index}").click(function(){
    	    	var rowid = $(this).attr("id");
    	    	var data = jQuery("#reservationList").jqGrid("getRowData",rowid);
    			window.parent.Boxy.confirm("<msg:message code='info.prompt.delete.confirm'/>",  function(){
    				var url = "${base}${res_restore_delete.controller}.json?c=${res_restore_delete.code}&ids="+rowid+"&prompt=name&name="+encodeURIComponent(encodeURIComponent(data.name));
    				$.ajax({
    					url:url,
    					type:'post',
    					timeout:'60000',
    ...
    $("#${res_restore.index}","#t_reservationList").click(function(){
		...
		if (selrow != null && selrow != ""){
			window.parent.Boxy.confirm("<msg:message code='info.prompt.restore.confirm'/>",  function(){
				var url = "${base}${res_restore.controller}.json?c=${res_restore.code}&delState=0&ids="+selrow+"&prompt=name&name="+encodeURIComponent(encodeURIComponent(names));
				$.ajax({
					url:url,
					type:'post',
					timeout:'60000',
    ...
    <security:authorize ifAnyGranted="${res_restore_delete.code}">
    var $ea = $("<a></a>").attr("href","javascript:void(0)")
			  .attr("id","${res_restore_delete.index}")
			  .append($("<img/>").attr("src","${base}/${common}/${style}/images/icon/delete.png")
			  .attr("width","18").attr("height","18").attr("border","0")
			  .attr("border","0").attr("align","absmiddle"))
			  .append("${res_restore_delete.name}");
	$("#t_reservationList").append("&nbsp;&nbsp;").append($("<span></span>").attr("class","jqgridContainer").append($ea));
    $("#${res_restore_delete.index}","#t_reservationList").click(function(){
    	var selrow = jQuery("#reservationList").jqGrid("getGridParam","selarrrow");
    	var names = "";
    	for(var i=0;i < selrow.length;i++){
    		var id = selrow[i];
		...
		if (selrow != null && selrow != ""){
			window.parent.Boxy.confirm("<msg:message code='info.prompt.delete.confirm'/>",  function(){
				var url = "${base}${res_restore_delete.controller}.json?c=${res_restore_delete.code}&ids="+selrow+"&prompt=name&name="+encodeURIComponent(encodeURIComponent(names));
				$.ajax({
					url:url,
					type:'post',
					timeout:'60000',
