...
$(document).ready(function(){
    jQuery("#investmentList").jqGrid({
        ...
        colModel: [{name: 'name',index:'name',width:'20%',align:'center',sortable:false},
                   ...
                   {name: 'productType',index:'productType',width:'20%',align:'center',sortable:false},
    	...
    	gridComplete: function(){
    		...
    		for(var i=0;i < ids.length;i++){
    			var id = ids[i];
    			var content = "";
    			<c:if test="${!empty investment_restore}">
    		    <security:authorize ifAnyGranted="${investment_restore.code}">
    			content += "<a href='javascript:void(0);' id='" + id + "' class='shortcut_${investment_restore.index}'";
    			content += "title='${investment_restore.name}' >";
    			content += "<img src='${base}/${common}/${style}/images/icon/restore.png' weight='18' height='18' border='0' align='absmiddle'/>${investment_restore.name}";
    			content += "</a>";
    			</security:authorize>
    			</c:if>
    			<c:if test="${!empty investment_restore_delete}">
    		    <security:authorize ifAnyGranted="${investment_restore_delete.code}">
    			content += "<a href='javascript:void(0);' id='" + id + "' class='shortcut_${investment_restore_delete.index}'";
    			content += "title='${investment_restore_delete.name}' >";
    			content += "<img src='${base}/${common}/${style}/images/icon/delete.png' weight='18' height='18' border='0' align='absmiddle'/>${investment_restore_delete.name}";
    			content += "</a>";
    			</security:authorize>
    			</c:if>
    	...
    	loadComplete:function(){
    	    $(".shortcut_${investment_restore.index}").click(function(){
    			...
    			window.parent.Boxy.confirm("<msg:message code='info.prompt.restore.confirm'/>",  function(){
    				var url = "${base}${investment_restore.controller}.json?c=${investment_restore.code}&state=0&ids="+rowid+"&prompt=name&name="+encodeURIComponent(encodeURIComponent(data.name));
    				$.ajax({
    					url:url,
    					type:'post',
    					timeout:'60000',
    	    ...
    	    $(".shortcut_${investment_restore_delete.index}").click(function(){
    	    	var rowid = $(this).attr("id");
    	    	var data = jQuery("#investmentList").jqGrid("getRowData",rowid);
    			window.parent.Boxy.confirm("<msg:message code='info.prompt.delete.confirm'/>",  function(){
    				var url = "${base}${investment_restore_delete.controller}.json?c=${investment_restore_delete.code}&ids="+rowid+"&prompt=name&name="+encodeURIComponent(encodeURIComponent(data.name));
    				$.ajax({
    					url:url,
    					type:'post',
    					timeout:'60000',
    ...
    $("#${investment_restore.index}","#t_investmentList").click(function(){
		...
		if (selrow != null && selrow != ""){
			window.parent.Boxy.confirm("<msg:message code='info.prompt.restore.confirm'/>",  function(){
				var url = "${base}${investment_restore.controller}.json?c=${investment_restore.code}&state=0&ids="+selrow+"&prompt=name&name="+encodeURIComponent(encodeURIComponent(names));
				$.ajax({
					url:url,
					type:'post',
					timeout:'60000',
    ...
    <security:authorize ifAnyGranted="${investment_restore_delete.code}">	 
    var $ea = $("<a></a>").attr("href","javascript:void(0)")
			  .attr("id","${investment_restore_delete.index}")
			  .append($("<img/>").attr("src","${base}/${common}/${style}/images/icon/delete.png")
			  .attr("width","18").attr("height","18").attr("border","0")
			  .attr("border","0").attr("align","absmiddle"))
			  .append("${investment_restore_delete.name}");
	$("#t_investmentList").append("&nbsp;&nbsp;").append($("<span></span>").attr("class","jqgridContainer").append($ea));    
    $("#${investment_restore_delete.index}","#t_investmentList").click(function(){
    	var selrow = jQuery("#investmentList").jqGrid("getGridParam","selarrrow");
    	var names = "";
    	for(var i=0;i < selrow.length;i++){
    		var id = selrow[i];
		...
		if (selrow != null && selrow != ""){
			window.parent.Boxy.confirm("<msg:message code='info.prompt.delete.confirm'/>",  function(){
				var url = "${base}${investment_restore_delete.controller}.json?c=${investment_restore_delete.code}&ids="+selrow+"&prompt=name&name="+encodeURIComponent(encodeURIComponent(names));
				$.ajax({
					url:url,
					type:'post',
					timeout:'60000',
