...
$(document).ready(function(){
    jQuery("#salonFeedbackList").jqGrid({
    	...
    	gridComplete: function(){
    		...
    		for(var i=0;i < ids.length;i++){
    		    ...
    		    <security:authorize ifAnyGranted="${restore.code}">
    			content += "<a href='javascript:void(0);' id='" + id + "' class='shortcut_${restore.index}'";
    			content += "title='${restore.name}' >";
    			content += "<img src='${base}/${common}/${style}/images/icon/restore.png' weight='18' height='18' border='0' align='absmiddle'/>${restore.name}";
    			content += "</a>";
    			</security:authorize>
    			</c:if>
    			<c:if test="${!empty restore_delete}">
    		    <security:authorize ifAnyGranted="${restore_delete.code}">
    			content += "<a href='javascript:void(0);' id='" + id + "' class='shortcut_${restore_delete.index}'";
    			content += "title='${restore_delete.name}' >";
    			content += "<img src='${base}/${common}/${style}/images/icon/delete.png' weight='18' height='18' border='0' align='absmiddle'/>${restore_delete.name}";
    			content += "</a>";
    			</security:authorize>
    			</c:if>
    	...
    	loadComplete:function(){
    	    $(".shortcut_${restore_delete.index}").click(function(){
    			...
    			window.parent.Boxy.confirm("<msg:message code='info.prompt.delete.confirm'/>",  function(){
    				var url = "${base}${restore_delete.controller}.json?c=${restore_delete.code}&ids="+rowid+"&prompt=title&title="+encodeURIComponent(encodeURIComponent(data.title));
    				$.ajax({
    					url:url,
    					type:'post',
    					timeout:'60000',
    	    ...
    	    $(".shortcut_${restore.index}").click(function(){
    	    	var rowid = $(this).attr("id");
    	    	var data = jQuery("#salonFeedbackList").jqGrid("getRowData",rowid);
    			window.parent.Boxy.confirm("<msg:message code='info.prompt.restore.confirm'/>",  function(){
    				var url = "${base}${restore.controller}.json?c=${restore.code}&status=0&ids="+rowid+"&prompt=title&title="+encodeURIComponent(encodeURIComponent(data.title));
    				$.ajax({
    					url:url,
    					type:'post',
    					timeout:'60000',
    ...
    <c:if test="${!empty restore_export}">
    <security:authorize ifAnyGranted="${restore_export.code}">	 
    var $ea = $("<a></a>").attr("href","javascript:void(0)")
    		  .attr("id","${restore_export.index}")
    		  .append($("<img/>").attr("src","${base}/${common}/${style}/images/icon/EXPort.png")
    		  .attr("width","18").attr("height","18").attr("border","0")
    		  .attr("border","0").attr("align","absmiddle"))
    		  .append("${restore_export.name}");
    ...
    $("#${restore_export.index}","#t_salonFeedbackList").click(function(){
    	var ids = jQuery("#salonFeedbackList").jqGrid('getDataIDs');
		if(ids.length > 0){
			window.location.href = "${base}${restore_export.controller}?c=${restore_export.code}&status=1&title="+title
    ...
    <security:authorize ifAnyGranted="${restore.code}">	 
    var $ea = $("<a></a>").attr("href","javascript:void(0)")
			  .attr("id","${restore.index}")
			  .append($("<img/>").attr("src","${base}/${common}/${style}/images/icon/restore.png")
			  .attr("width","18").attr("height","18").attr("border","0")
			  .attr("border","0").attr("align","absmiddle"))
			  .append("${restore.name}");
	$("#t_salonFeedbackList").append("&nbsp;&nbsp;").append($("<span></span>").attr("class","jqgridContainer").append($ea));    
    $("#${restore.index}","#t_salonFeedbackList").click(function(){
		...
		if (selrow != null && selrow != ""){
			window.parent.Boxy.confirm("<msg:message code='info.prompt.restore.confirm'/>",  function(){
				var url = "${base}${restore.controller}.json?c=${restore.code}&status=0&ids="+selrow+"&prompt=title&title="+encodeURIComponent(encodeURIComponent(titles));
				$.ajax({
					url:url,
					type:'post',
					timeout:'60000',
    ...
    <security:authorize ifAnyGranted="${restore_delete.code}">	 
    var $ea = $("<a></a>").attr("href","javascript:void(0)")
			  .attr("id","${restore_delete.index}")
			  .append($("<img/>").attr("src","${base}/${common}/${style}/images/icon/delete.png")
			  .attr("width","18").attr("height","18").attr("border","0")
			  .attr("border","0").attr("align","absmiddle"))
			  .append("${restore_delete.name}");
	$("#t_salonFeedbackList").append("&nbsp;&nbsp;").append($("<span></span>").attr("class","jqgridContainer").append($ea));    
    $("#${restore_delete.index}","#t_salonFeedbackList").click(function(){
    	var selrow = jQuery("#salonFeedbackList").jqGrid("getGridParam","selarrrow");
    	var titles = "";
    	for(var i=0;i < selrow.length;i++){
    		var id = selrow[i];
		...
		if (selrow != null && selrow != ""){
			window.parent.Boxy.confirm("<msg:message code='info.prompt.delete.confirm'/>",  function(){
				var url = "${base}${restore_delete.controller}.json?c=${restore_delete.code}&ids="+selrow+"&prompt=title&title="+encodeURIComponent(encodeURIComponent(titles));
				$.ajax({
					url:url,
					type:'post',
					timeout:'60000',
						...
						<table cellpadding="0" cellspacing="0" border="1" width="100%"  class="skinLayout">
							<tr>
								...
								<form:select path="isJoin" >
									...
									<form:option value="2" ><msg:message code='salonFeedback.noJoin'/></form:option>
								</form:select>
								</td>
